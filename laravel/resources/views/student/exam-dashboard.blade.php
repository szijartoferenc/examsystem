...
@section('space-work')
      ...
      <div class="container">
            <p style="color:black">Üdvözöljük, {{Auth::user()->name}}</p>
            ...
            @if ($success == true)
                ...
                @if (count($qna) > 0)
                    ...
                    @csrf
                        <input type="hidden" name="exam_id" value="{{$exam[0]['id']}}">
                                @foreach ($qna as $data)
                                    <div>
                                        <h5>K{{$qcount++}}. {{ $data['question'][0]['question']}}</h5>
                ...
                @else
                    <h3 style="color:red;" class="text-center">Kérdések és válaszok nem elérhetőek</h3>
                @endif
      ...
      <script>
        $(document).ready(function(){
            ...
            var time = @json($time);
            $('.time').text(time[0]+':'+time[1]+':00 Maradék idő');
          ...
          var timer = setInterval(() => {
                ...
                $('.time').text(tempHours+':'+tempMinutes+':'+tempSeconds+'  Maradék idő');
        ...
        function isValid(){
            ...
            for(let i = 1; i <= qlength; i++){
                if ($('#ans_'+i).val() == "") {
                        result = false;
                        $('#ans_'+i).parent().append('<span style="color:red;" class="error_msg">Kérem válassza ki a választ!</span>')
                        setTimeout(() => {
                            $('.error_msg').remove();
                        }, 5000);
                }
